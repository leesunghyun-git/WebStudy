	...
	<div class="container">
		<div class="row">
			...
			<table class="table">
				<tr>
					<td>
						<a href="../board/insert.do" class="btn btn-primary">새글</a>
					</td>
				</tr>
			</table>
			 		...
			 		<c:forEach var="vo" items="${list }">
			 			<tr>
			 				...
			 				<c:if test="${vo.group_tab > 0}">
			 					...
			 					<img src="../replyboard/re_icon.png" class="reply-icon">
			 				</c:if>
			 				<c:if test="${msg!=vo.subject }">
			 				<a href="../board/detail.do?no=${vo.no }" >${vo.subject }</a>
			 				</c:if>
			 				<c:if test="${msg==vo.subject }">
			 				<span><font color="gray">${msg}</font></span>
			 				</c:if>
			 				<c:if test="${vo.dbday eq today }">
